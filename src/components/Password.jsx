...
function Password() {
    ...
    const passwordStrength = () => {
      ...
      const setClass = (oass, i) => {
        const baseClass = (
          oass === 1 && i <= 1 ? 'bg-red' :
          oass === 2 && i <= 2 ? 'bg-orange' :
          oass === 3 && i <= 3 ? 'bg-yellow' :
          oass === 4 ? 'bg-green' : ''
        );
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen ">
        <div className="w-1/3">
                <h1 className="text-2xl text-center text-gray-darker">Password Generator</h1>
            <div className="flex items-center justify-between bg-mainBg-dark px-6 py-4 mt-8 mb-4">
                    <span id="generatedPass" className="text-2xl text-gray-dark opacity-25">{generated}</span>
                <button id="copyPass" className="text-green"  onClick={() => copyPassword()}>  
            ...
            <span id="length" className="text-green text-2xl">{length}</span>
            </div>
            <div className="py-4">
                <input
                    type="range" 
            ...
            <div className="relative flex items-center gap-4">
                ...
                />
                <label htmlFor="uppercase" className="text-gray-dark relative cursor-pointer label-check">Include Uppercase Letters (A-Z)</label>
            </div>
            <div className="relative flex items-center gap-4">
                <input
                    className="input-check"
                ...
                />
                <label htmlFor="lowercase" className="text-gray-dark relative cursor-pointer label-check">Include Lowercase Letters (a-z)</label>
            </div>
            <div className="relative flex items-center gap-4">
                <input
                    className="input-check"
                ...
                />
                <label htmlFor="numbers" className="text-gray-dark relative cursor-pointer label-check">Include Numbers (0-9)</label>
            </div>
            <div className="relative flex items-center gap-4">
                <input
                    className="input-check"
                ...
                />
                <label htmlFor="symbols" className="text-gray-dark relative cursor-pointer label-check">Include Symbols (!?*; etc..)</label>
            </div>
            </div>
            ...
            <div id="strength" className="flex items-center gap-3">
                <h3 id="status" className="text-white text-2xl uppercase">{setStrong(strengthValue)}</h3>
                <div className="flex gap-1">
                    {Array.from({ length: 4 }, (_, index) => (
                        <span key={index} className={setClass(strengthValue, index + 1)} ></span>
                    ))}
                </div>
            </div>
            </div>
            <button id="generator" className="bg-green w-full py-4 flex justify-center items-center gap-4 text-mainBg-darker hover:bg-mainBg-darker hover:text-green transition-all border border-green" onClick={setGeneratedEl}>
            ...
            <p className={ `${copyPass ? 'top-2'  : '-top-16'} fixed right-8 py-4 px-8 text-black transition-all bg-green`} id="message" role="alert">copied</p>
        </div>
        </div>
    </div>
